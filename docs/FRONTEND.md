...
Имя таблицы передаётся в сообщении аутентификации и должно соответствовать
формату `^[a-z][a-z0-9_]{0,62}$`; одна WebSocket-сессия обслуживает одну таблицу.
Владелец lock хранится на сервере: только соединение-владелец может отправить
